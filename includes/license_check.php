...
// Compatível com MySQL (local) e PostgreSQL/Neon (público)
...
function verificarLicencaAtiva() {
    ...
    try {
        // ============================================================
        // DETECTAR O TIPO DE BANCO
        // ============================================================
        $db_type = defined('DB_TYPE') ? DB_TYPE : 'mysql';
        $is_pgsql = ($db_type === 'pgsql');
        
        // ============================================================
        // VERIFICAR SE A TABELA EXISTE (sintaxe por banco)
        // ============================================================
        if ($is_pgsql) {
            // PostgreSQL
        ...
        // ============================================================
        // QUERY PARA BUSCAR LICENÇA ATIVA (varia por banco)
        // ============================================================
        if ($is_pgsql) {
            ...
            $sql = "
                SELECT 
                    ...
                    data_ativacao,
                    data_expiracao,
                    EXTRACT(EPOCH FROM (data_expiracao::timestamp - NOW()))::bigint as segundos_restantes,
                    status
                FROM licencas 
        ...
        // ============================================================
        // Garantir que segundos_restantes é inteiro
        // ============================================================
        $segundos = (int)$licenca['segundos_restantes'];
        
        // ============================================================
        // Verificar se expirou
        // ============================================================
        if ($segundos <= 0) {
            $pdo->prepare("UPDATE licencas SET status = 'expirada' WHERE id = :id")
                ->execute([':id' => $licenca['id']]);
        ...
        // ============================================================
        // Alerta para licença de teste expirando
        // ============================================================
...
// ============================================
// FUNÇÃO PARA FORMATAR DATA (MySQL e PostgreSQL)
// ============================================
function formatarDataExpiracao($data) {
    if (empty($data)) return '';
    
    // strtotime() lida com 'Y-m-d H:i:s' (MySQL) e ISO 8601 (PostgreSQL)
        ...
        <div class="license-block">
            ...
            <div class="alert-box">
                ...
                <p style="color: #4a5568; font-size: 14px; margin-top: 5px;">
                    Expirada em: <span class="date"><?= formatarDataExpiracao($licenca_info['expiracao']) ?></span>
                </p>
                <?php endif; ?>
                <p style="color: #4a5568; font-size: 14px; margin-top: 5px;">
                    Status: <span style="color: #e74c3c; font-weight: 600;"><?= ucfirst($licenca_info['status'] ?? 'bloqueado') ?></span>
                </p>
                <?php if (isset($licenca_info['mensagem'])): ?>
                <p style="color: #991b1b; font-size: 12px; margin-top: 10px; word-break: break-all;">
                    <?= htmlspecialchars($licenca_info['mensagem']) ?>
                </p>
                <?php endif; ?>
            </div>
