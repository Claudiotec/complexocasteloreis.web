<?php
// ============================================
// modules/escola/disciplinas/add.php - Cadastrar Disciplina
// ============================================
...
require_once $base_path . '/coordenador_pedagogico/verificar_permissao.php';
...
// REMOVA estas linhas duplicadas:
// require_once '../../../config/database.php';
// require_once '../../../config/app_modes.php';

// Verificação de login já feita pelo database.php
...
try {
    ...
    // Tenta com a tabela funcionarios se forca_trabalho não existir
    try {
        $professores = $pdo->query("SELECT id, nome, categoria_actual, funcao_instituicao 
                                   FROM funcionarios 
                                   WHERE status = 'ativo' 
...
$turmas = [];
try {
    $turmas = $pdo->query("SELECT id, nome, sala, turno, ano_letivo, classe, curso 
                           FROM turmas 
                           WHERE status = 'ativa' 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        // Inserir disciplina
        $stmt = $pdo->prepare("INSERT INTO disciplinas (nome, descricao, carga_horaria, professor_id, status) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([
            $_POST['nome'],
            $_POST['descricao'] ?? '',
...
<div class="form-container">
    ...
    <form method="POST">
        <div class="form-group">
            ...
            <input type="text" name="nome" value="<?= htmlspecialchars($_POST['nome'] ?? '') ?>" required placeholder="Digite o nome da disciplina">
        </div>
        
        <div class="form-group">
            <label>Descrição</label>
            <textarea name="descricao" rows="3" placeholder="Descreva a disciplina"><?= htmlspecialchars($_POST['descricao'] ?? '') ?></textarea>
        </div>
        
        <div class="form-row">
            <div class="form-group">
                <label>Carga Horária (horas)</label>
                <input type="number" name="carga_horaria" value="<?= $_POST['carga_horaria'] ?? '' ?>" min="1" placeholder="Ex: 45">
            </div>
            <div class="form-group">
                    ...
                    <?php foreach($turmas as $t): ?>
                        <div class="checkbox-item">
                            <input type="checkbox" 
                                   ...
                                   name="turmas[]" 
                                   value="<?= $t['id'] ?>"
                                   <?= (isset($_POST['turmas']) && in_array($t['id'], $_POST['turmas'])) ? 'checked' : '' ?>>
        ...
        <div class="form-group">
            ...
            <select name="status">
                ...
                <option value="inativa" <?= ($_POST['status'] ?? '') == 'inativa' ? 'selected' : '' ?>>Inativa</option>
            </select>
        </div>
