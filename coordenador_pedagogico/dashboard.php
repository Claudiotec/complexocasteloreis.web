...
// Dashboard do Coordenador Pedagógico - COMPLETO CORRIGIDO
// ============================================
...
$isCoordenador = in_array($perfil, ['coordenador_pedagogico', 'coordenador', 'admin']);
...
// ==========================================
function tabelaTemDados($pdo, $tabela) {
    try {
...
try {
    ...
    // ==========================================
    // BUSCAR TURMAS - COORDENADOR VÊ TODAS
    // ==========================================
    try {
        if ($isCoordenador) {
            ...
            $totalTurmas = count($turmasAtivas);
            ...
            if ($professor_num_agente) {
                $stmt = $pdo->prepare("
                    SELECT t.*, 
                           (SELECT COUNT(*) FROM alunos a WHERE a.TURMA = t.nome AND a.status = 'ativo') as total_alunos
                    FROM turmas t
                    WHERE t.id IN (SELECT turma_id FROM destribuicao_professores WHERE professor_id = ? AND tipo = 'PROFESSOR')
                    AND t.status = 'ativa'
                    ORDER BY t.classe, t.nome
                ");
        ...
        // Se não encontrou turmas, tenta buscar todas mesmo assim (fallback para admin)
        if (empty($turmasAtivas) && ($isCoordenador || $perfil == 'admin')) {
            $stmt = $pdo->query("
                SELECT t.*, 
                       (SELECT COUNT(*) FROM alunos a WHERE a.TURMA = t.nome AND a.status = 'ativo') as total_alunos
                FROM turmas t
    ...
    try {
        ...
        // Se não encontrou turmas, tenta buscar todas mesmo assim (fallback)
    ...
    // ===== ATUALIZAR TURMAS PARA O SELECT =====
    // Esta variável é usada no filtro de turmas
    ...
    if (tabelaExiste($pdo, 'disciplinas') && tabelaTemDados($pdo, 'disciplinas')) {
        try {
            $stmt = $pdo->query("SELECT * FROM disciplinas WHERE status = 'ativa' ORDER BY nome LIMIT 50");
    ...
    // ===== Distribuições =====
    if (tabelaExiste($pdo, 'destribuicao_professores') && tabelaTemDados($pdo, 'destribuicao_professores')) {
        try {
            $stmt = $pdo->query("
            ...
            // Tenta sem o join
            $stmt = $pdo->query("SELECT * FROM destribuicao_professores ORDER BY id DESC LIMIT 50");
            $distribuicoes = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach ($distribuicoes as &$d) {
                $d['professor_nome'] = $d['professor_nome'] ?? 'N/A';
            }
        }
    }
    ...
    // ===== Notas =====
    if (tabelaExiste($pdo, 'notas_alunos') && tabelaTemDados($pdo, 'notas_alunos')) {
        try {
            $stmt = $pdo->query("
    ...
    // ===== Frequência =====
    if (tabelaExiste($pdo, 'frequencia') && tabelaTemDados($pdo, 'frequencia')) {
        try {
            $stmt = $pdo->query("
    ...
    // ===== Logs =====
    // Não buscar logs se a tabela não existir
                ...
                <div class="card-modern">
                    ...
                    <div class="card-body">
                        <div class="quick-actions">
                            ...
                            <a href="#" class="quick-action" onclick="abrirTemplate('lançar_notas')">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('📊 Turmas carregadas: <?= count($turmasAtivas) ?>');
...
function showSection(section) {
    ...
    for (var i = 0; i < links.length; i++) {
        ...
        if (links[i].getAttribute('onclick') && links[i].getAttribute('onclick').includes(section)) {
    ...
    if (titleEl) {
        titleEl.textContent = titles[section] || section.charAt(0).toUpperCase() + section.slice(1);
    }
...
function abrirTemplate(template) {
    var urls = {
        ...
        'lançar_notas': '/softgest_web/coordenador_pedagogico/templates/lançar_notas.php',
    ...
    if (!url) {
        alert('⚠️ Template não encontrado: ' + template);
        return;
    }
...
function abrirMensagens() {
    var url = '/softgest_web/coordenador_pedagogico/templates/mensagens.php';
    window.open(url, '_blank');
}
...
function abrirProfessores() {
    var url = '/softgest_web/coordenador_pedagogico/templates/professores/index.php';
    window.open(url, '_blank');
}
...
function verDetalhesTurma(id) {
    alert('🔍 Detalhes da turma #' + id + ' (em desenvolvimento)');
}

function editarTurma(id) {
    alert('✏️ Editar turma #' + id + ' (em desenvolvimento)');
}

function gerarPautaTurma(id) {
    window.open('/softgest_web/coordenador_pedagogico/templates/pautas_finais.php?turma=' + id, '_blank');
}

function gerarBoletim() {
    var aluno = document.getElementById('selectAlunoBoletim');
    if (aluno && aluno.value) {
        window.open('/softgest_web/coordenador_pedagogico/templates/boletins/boletins.php?aluno_id=' + aluno.value, '_blank');
    } else {
...
function salvarAluno() {
    alert('💾 Salvando aluno... (em desenvolvimento)');
}

function salvarTurma() {
    alert('💾 Salvando turma... (em desenvolvimento)');
}

function salvarNota() {
    alert('💾 Salvando nota... (em desenvolvimento)');
}

function salvarPrazo() {
    alert('💾 Salvando prazo... (em desenvolvimento)');
}

function exportarDistribuicaoExcel() {
    alert('📊 Exportando para Excel... (em desenvolvimento)');
}

function gerarRelatorioDistribuicao() {
    alert('📄 Gerando relatório... (em desenvolvimento)');
}

function exportarEstatisticasPDF() {
    alert('📄 Exportando PDF... (em desenvolvimento)');
}

function exportarEstatisticasExcel() {
    alert('📊 Exportando Excel... (em desenvolvimento)');
}
...
function abrirModalPrazo() {
    var modal = document.getElementById('modalPrazo');
    if (modal) {
        // Mostrar modal manualmente se Bootstrap não estiver carregado
...
function mostrarNotificacao(mensagem, tipo) {
    ...
    // Tenta usar o sistema de notificações se disponível
    try {
        ...
        var cores = { 'info': '#3498db', 'success': '#27ae60', 'warning': '#f39c12', 'danger': '#e74c3c' };
        var icones = { 'info': 'ℹ️', 'success': '✅', 'warning': '⚠️', 'danger': '❌' };
        
        var toast = document.createElement('div');
        toast.className = 'notification-toast ' + (tipo || 'info');
        toast.style.cssText = 'background:white;border-left:4px solid ' + (cores[tipo] || '#3498db') + ';border-radius:8px;box-shadow:0 4px 15px rgba(0,0,0,0.15);margin-bottom:10px;padding:12px 15px;cursor:pointer;animation:slideIn 0.3s ease;';
        toast.innerHTML = '<div style="display:flex;align-items:start;gap:10px;"><div style="font-size:1.2rem;">' + (icones[tipo] || 'ℹ️') + '</div><div style="flex:1;"><div style="font-weight:bold;">' + (tipo === 'danger' ? 'Alerta' : 'Notificação') + '</div><div style="font-size:0.85rem;color:#666;">' + mensagem + '</div><div style="font-size:0.7rem;color:#999;margin-top:5px;">' + new Date().toLocaleTimeString() + '</div></div><button style="font-size:0.7rem;background:none;border:none;cursor:pointer;" onclick="this.parentElement.parentElement.remove()">✕</button></div>';
...
function initCharts() {
    try {
        // Tentar carregar Chart.js se disponível
        if (typeof Chart !== 'undefined') {
            // Gráfico de classes
            var ctxClasses = document.getElementById('chartClasses');
            if (ctxClasses) {
                var classes = {};
                <?php foreach ($alunosList as $aluno): ?>
                    var classe = '<?= addslashes($aluno['Classe'] ?? 'N/A') ?>';
                    classes[classe] = (classes[classe] || 0) + 1;
                <?php endforeach; ?>
                
                var labels = Object.keys(classes);
                var data = Object.values(classes);
        ...
        console.log('Erro ao carregar gráficos:', e);
    }
}
...
// ============================================
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Dashboard do Coordenador Pedagógico iniciado!');
    ...
    console.log('📚 Turmas ativas: <?= count($turmasAtivas) ?>');
