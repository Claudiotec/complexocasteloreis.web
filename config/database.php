<?php
// config/database.php
// Cria a conexão $pdo global e define funções auxiliares
...
if (!isset($pdo) || !($pdo instanceof PDO)) {
    try {
        if (function_exists('conectarBanco')) {
            ...
            throw new Exception("Função conectarBanco() não encontrada. Verifique app_modes.php");
        }
    } catch (Exception $e) {
        error_log("❌ Erro ao conectar banco em database.php: " . $e->getMessage());
        
        // Em modo local, mata a página com mensagem clara
        if (PHP_OS_FAMILY === 'Windows' || getenv('RENDER') === false) {
...
// Garantir que $pdo está acessível globalmente
$GLOBALS['pdo'] = $pdo;
...
// SITE_URL já é definido em config/paths.php
// Este bloco só roda se paths.php NÃO foi carregado ainda
if (!defined('SITE_URL')) {
    if (PHP_OS_FAMILY === 'Windows') {
