...
$categorias = $pdo->query("SELECT * FROM categorias_caixa WHERE tipo = 'entrada' ORDER BY nome")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO movimentacoes_caixa (tipo, categoria, descricao, valor, data_movimento, forma_pagamento, cliente_id, observacoes, status) VALUES ('entrada', ?, ?, ?, ?, ?, ?, ?, 'confirmado')");
        $stmt->execute([$categoria, $descricao, $valor, $data_movimento, $forma_pagamento, $cliente_id, $observacoes]);
...
<head>
    ...
    <title>Nova Entrada - SoftGest Web</title>
    ...
    <style>
        ...
        .form-card::before {
            ...
            height: 5px; background: linear-gradient(90deg, #2ecc71, #27ae60);
        }
        .form-card h2 { color: #1a2332; border-bottom: 2px solid #f0f2f5; padding-bottom: 15px; margin-bottom: 25px; }
        ...
        .form-group input:focus, .form-group select:focus, .form-group textarea:focus {
            border-color: #2ecc71; outline: none; box-shadow: 0 0 0 3px rgba(46,204,113,0.1);
        }
        .form-group textarea { resize: vertical; min-height: 60px; }
        .form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
        .btn-success {
            background: #2ecc71; color: white; padding: 14px 30px; border: none;
            border-radius: 8px; font-size: 16px; font-weight: 700; cursor: pointer;
            transition: all 0.3s;
        }
        .btn-success:hover { transform: translateY(-2px); box-shadow: 0 4px 15px rgba(46,204,113,0.3); }
        ...
        <div class="form-card">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="descricao" required placeholder="Descrição da entrada">
                </div>
