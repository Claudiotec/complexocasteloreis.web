...
try {
    ...
    // ============================================
    // 4. IDs DOS EMOLUMENTOS DE TRANSPORTE
    // ============================================
    $emolumento_transporte_ids = [19, 56, 66, 102, 104, 114];
    
    // ============================================
    // 5. CONSULTA: BUSCAR ALUNOS PELO NOME (IGNORANDO ID)
    // ============================================
    ...
    // ============================================
    // 6. PARA CADA PAGAMENTO, BUSCAR O ALUNO PELO NOME
    // ============================================
    $alunos_encontrados = [];
    ...
    foreach ($pagamentos as $pag) {
        ...
        if (!$aluno && !empty($nome_pagamento)) {
            ...
            // Se ainda não encontrou, tentar com o nome exato sem LIKE
            if (!$aluno) {
...
<style>
    ...
    .print-content .empty-print {
        text-align: center;
        padding: 40px 20px;
        color: #888;
    }
    
    .print-content .empty-print .icon {
        font-size: 40px;
        display: block;
        margin-bottom: 10px;
    }
    ...
    <?php if (count($alunos_agrupados) > 0): ?>
        ...
        <div class="lista-card" data-classe="<?= htmlspecialchars($classe) ?>" data-turma="<?= htmlspecialchars($turma) ?>">
            ...
            foreach($alunos_lista as $aluno): 
                $data_nasc = '';
                if (isset($aluno['dia']) && isset($aluno['mes']) && isset($aluno['Ano']) && $aluno['dia'] > 0) {
        ...
        <div style="grid-column: 1 / -1;">
            <div class="empty-state">
                ...
                <div class="debug-info">
                    ...
                    <p><strong>Total de alunos ativos:</strong> 343</p>
...
function gerarHTMLImpressao(cards) {
    ...
    cards.forEach(function(card) {
        ...
        var alunos = card.querySelectorAll('.aluno-item');
        ...
        alunos.forEach(function(item) {
            var nome = item.querySelector('.nome')?.textContent?.trim() || '';
            // Remover o número e o badge
            nome = nome.replace(/^\d+\.\s*/, '');
            nome = nome.replace(/[MF]\s*$/, '').trim();
            
            var sexo = item.querySelector('.badge-sexo')?.textContent?.trim() || 'M';
            var dados = item.querySelector('.dados')?.textContent?.trim() || 'Nasc: - | - anos';
            ...
            html += `
                <tr>
                    ...
                    <td>${dados.replace('Nasc: ', '').replace(' | ', ' | ')}</td>
                    <td>${dados.split('|')[1]?.trim() || '-'}</td>
                </tr>
            `;
            num++;
        });
