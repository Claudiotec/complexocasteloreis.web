...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        // CORRIGIDO: status = 'paga' (com "a" no final)
        $stmt = $pdo->prepare("
            UPDATE contas 
            SET status = 'paga', 
                data_pagamento = ?, 
            ...
            WHERE id = ?
        ");
        $stmt->execute([$data_pagamento, $forma_pagamento, $data_pagamento, $forma_pagamento, $id]);
    ...
    <div class="info-box">
        <div class="row">
            ...
            <div>
                ...
                <div class="value destaque">Kz <?= number_format($conta['valor'], 2, ',', '.') ?></div>
            </div>
            <div>
