...
function verificarDuplicadoDetalhado($pdo, $aluno_id, $emolumento_id, $mes_referencia, $nome_aluno = null) {
    try {
        // Se não tem aluno_id e não tem nome_aluno, não tem como verificar
        if (empty($aluno_id) && empty($nome_aluno)) {
            return ['duplicado' => false];
        }
        
        // 🔥 CORREÇÃO PRINCIPAL: Buscar pagamentos pelo emolumento e mês
        // E filtrar pelo aluno_id OU nome_aluno
        ...
        // 🔥 CORREÇÃO: SEMPRE filtrar por aluno_id se existir
        if (!empty($aluno_id)) {
            // Pagamento normal - tem aluno_id
            $sql .= " AND p.aluno_id = ?";
            $params[] = $aluno_id;
        } else if (!empty($nome_aluno)) {
            // Pagamento manual - verifica por nome
            // Primeiro tenta encontrar o aluno pelo nome exato
            ...
            if ($aluno_encontrado) {
                // Se o aluno existe, verifica pelo ID
                $sql .= " AND p.aluno_id = ?";
                $params[] = $aluno_encontrado['id'];
            } else {
                // Se não existe, verifica pelo nome salvo no pagamento
        ...
        if ($pagamento) {
            // Buscar o nome do aluno para exibição
        ...
        // 🔥 CORREÇÃO: Verificar também mensalidades (apenas para alunos cadastrados)
        if (!empty($aluno_id) && $mes_referencia != '-' && !empty($mes_referencia)) {
            ...
            if ($mes_numero > 0) {
                ...
                if ($mensalidade) {
                    ...
                    // Só considera duplicado se for propina ou mensalidade
    ...
    if (empty($itens_fatura)) {
        ...
        try {
            ...
            if (empty($itens_registrados) && !empty($itens_duplicados)) {
                ...
                $numero_fatura = 'FAT-' . date('Ymd') . '-' . str_pad($primeiro_pagamento_id ?? 1, 6, '0', STR_PAD_LEFT);
                $dados_empresa = buscarDadosEmpresa($pdo);
                ...
                if ($primeiro_pagamento_id) {
                    try {
                        ...
                        if ($pag_notif) {
                            $_SESSION['ultima_notificacao_mobile'] = [
                                'aluno_nome' => $pag_notif['aluno_nome'] ?? $aluno['nome'] ?? 'Aluno',
                                'valor' => number_format($pag_notif['valor'] ?? $total_pago, 2, ',', '.'),
                                'emolumento' => $pag_notif['emolumento_nome'] ?? 'Pagamento',
