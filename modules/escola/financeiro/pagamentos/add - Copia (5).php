...
// ============================================
// FUNÇÃO PARA BUSCAR DADOS DA EMPRESA DA TABELA empresa
// ============================================
...
// ============================================
// PROCESSAR PAGAMENTOS MANUAL - COM MÚLTIPLOS ITENS
// ============================================
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['salvar_pagamento_manual'])) {
    ...
    // Verificar se há itens na sessão
    $itens_manual = $_SESSION['fatura_itens_manual'] ?? [];
    if (empty($itens_manual)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id, nome, classe, turma FROM alunos WHERE nome = ? AND status = 'ativo' LIMIT 1");
            ...
            if ($aluno_existente) {
                ...
                $stmt_insert = $pdo->prepare("
                    INSERT INTO alunos (nome, Classe, TURMA, Periodo, Sexo, Situacao_Cadastro, status, created_at)
                    VALUES (?, ?, ?, ?, ?, 'Confirmação', 'ativo', NOW())
                ");
                $stmt_insert->execute([$aluno_nome, $aluno_classe, $aluno_turma, $aluno_periodo, $aluno_genero]);
                $aluno_id = $pdo->lastInsertId();
            }
            
            if (empty($aluno_id)) {
                throw new Exception('Não foi possível obter o ID do aluno.');
            }
            
            // 3. REGISTRAR CADA ITEM DA FATURA
            $total_pago = 0;
            $itens_registrados = [];
            $itens_duplicados = [];
            ...
            foreach ($itens_manual as $item) {
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO pagamentos (
                        aluno_id, emolumento_id, valor, data_pagamento, 
                        forma_pagamento, referencia, observacoes, 
                        status, mes_referencia, created_at
            ...
            if (empty($itens_registrados) && !empty($itens_duplicados)) {
                ...
                // 4. GERAR FATURA
                $numero_fatura = 'FAT-MANUAL-' . date('Ymd') . '-' . str_pad($pagamento_id ?? 1, 6, '0', STR_PAD_LEFT);
                $dados_empresa = buscarDadosEmpresa($pdo);
                ...
                $sucesso .= "👤 Aluno: <strong>$aluno_nome</strong> (ID: #$aluno_id)<br>";
                ...
                $sucesso .= "📅 Data: " . date('d/m/Y', strtotime($data_pagamento));
                
                if (!empty($itens_duplicados)) {
                    $sucesso .= '<br>⚠️ ' . count($itens_duplicados) . ' item(ns) ignorado(s) por duplicidade.';
                }
                
                // Limpar itens da fatura manual
                $_SESSION['fatura_itens_manual'] = [];
                $itens_manual = [];
            }
            
        } catch (Exception $e) {
            $erro = 'Erro ao registrar pagamento manual: ' . $e->getMessage();
        }
    }
}
    ...
    <div class="manual-body">
        ...
        <form method="POST" action="" id="manualForm" style="margin-top:20px;border-top:2px solid #eef2f7;padding-top:20px;">
            ...
            <div class="info-valor" style="margin-top:10px;background:#fef9e8;border-color:#fde68a;">
                ⚠️ <strong>Pagamento Manual:</strong> O aluno será cadastrado automaticamente se não existir na base de dados.
                <?php if (!empty($itens_manual)): ?>
