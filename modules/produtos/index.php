<?php
// ============================================
// modules/produtos/index.php - Lista de Produtos
// ============================================
...
try {
    ...
    $valorEstoque = $pdo->query("SELECT SUM(quantidade * preco_venda) FROM produtos")->fetchColumn() ?? 0;
} catch (Exception $e) {}
...
<head>
    ...
    <title>Produtos - <?= SITE_NAME ?></title>
    ...
    <main class="main-content">
        ...
        <div class="content-area">
            <div class="page-header">
                <div>
                    <h1>📦 Produtos</h1>
                    <p class="subtitle">Cadastro e gestão de produtos</p>
                </div>
                <div class="actions">
                    <a href="add.php" class="btn btn-primary">➕ Novo Produto</a>
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card warning">
                    <span class="icon">💰</span>
                    <div class="number">R$ <?= number_format($valorEstoque, 2, ',', '.') ?></div>
                    <div class="label">Valor do Estoque</div>
                </div>
            </div>
                        ...
                        <?php if (count($produtos) > 0): ?>
                            ...
                            <?php 
                                ...
                                if ($produto['quantidade'] <= $produto['estoque_minimo']) {
                                    ...
                                    $statusLabel = '🔴 Crítico';
                                } elseif ($produto['quantidade'] <= $produto['estoque_minimo'] * 2) {
                                    $statusEstoque = 'baixo';
                                    $statusLabel = '🟡 Baixo';
                                }
                            ?>
                            <tr>
                                <td class="codigo"><?= htmlspecialchars($produto['codigo']) ?></td>
                                <td class="nome"><?= htmlspecialchars($produto['nome']) ?></td>
                                <td><?= htmlspecialchars($produto['categoria'] ?? '-') ?></td>
                                <td class="text-center"><?= $produto['quantidade'] ?></td>
                                <td class="valor">R$ <?= number_format($produto['preco_compra'], 2, ',', '.') ?></td>
                                <td class="valor">R$ <?= number_format($produto['preco_venda'], 2, ',', '.') ?></td>
                                <td><?= htmlspecialchars($produto['fornecedor'] ?? '-') ?></td>
                                <td>
                                    <span class="status-estoque status-<?= $statusEstoque ?>">
                                        <?= $statusLabel ?>
                                    </span>
                                </td>
                                <td>
                                    <div class="table-actions">
                                        ...
                                        <a href="view.php?id=<?= $produto['id'] ?>" class="btn btn-sm btn-info">Visualizar</a>
