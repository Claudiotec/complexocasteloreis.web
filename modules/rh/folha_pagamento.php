...
function calcularDescontos($salario, $config, $faltas = 0, $horasFalta = 0) {
    ...
    // Desconto por faltas (limitado ao salário)
    ...
    // Limitar desconto de faltas ao salário
    if ($descontoFaltas > $salario) {
        $descontoFaltas = $salario;
    }
    ...
    // Total de descontos (limitado ao salário para não ficar negativo)
    $totalDescontos = $inss + $irt + $saude + $descontoFaltas;
    
    // Garantir que o total de descontos não ultrapasse o salário
    if ($totalDescontos > $salario) {
        $totalDescontos = $salario;
    }
    
    // Salário líquido (nunca negativo)
...
$mes = $_GET['mes'] ?? date('m');
$ano = $_GET['ano'] ?? date('Y');
$diasUteisMes = $config_descontos['dias_uteis'];
...
foreach($funcionariosSalario as $func) {
    ...
    // Calcular descontos (com limite de salário)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ============================================
           HEADER DA EMPRESA - ESTILO PROFISSIONAL
           ============================================ */
        .empresa-header {
        ...
        .empresa-header .empresa-info .item strong {
            color: white;
            font-weight: 600;
        }
        ...
        .valor-zero { color: #95a5a6; font-weight: 400; }
        ...
        .total-geral .item .value.zero { color: #95a5a6; }
...
<body>
    ...
    <div class="main-content">
        <div class="container">
            ...
            <!-- ==========================================
            HEADER DA EMPRESA - PROFISSIONAL
            ========================================== -->
                    ...
                    <tbody>
                        <?php if (count($dadosFuncionarios) > 0): ?>
                            ...
                            <?php foreach($dadosFuncionarios as $func): 
                                $desc = $func['descontos'];
                                // Definir classe para o líquido
                                $liqClasse = 'valor-positivo';
                                if ($desc['liquido'] == 0) $liqClasse = 'valor-zero';
                            ?>
                            <tr>
                                <td><?= $contador++ ?></td>
                                ...
                                <td class="<?= $liqClasse ?>" style="font-weight: 700;">Kz <?= number_format($desc['liquido'], 2, ',', '.') ?></td>
            ...
            <div class="total-geral">
                ...
                <div class="item">
                    ...
                    <span class="value <?= $totalIrt > 0 ? 'negativo' : 'zero' ?>">Kz <?= number_format($totalIrt, 2, ',', '.') ?></span>
                </div>
                <div class="item">
                ...
                <div class="item">
                    ...
                    <span class="value <?= $totalLiquido > 0 ? 'positivo' : ($totalLiquido == 0 ? 'zero' : 'negativo') ?>">Kz <?= number_format($totalLiquido, 2, ',', '.') ?></span>
                </div>
            </div>
            ...
            <div class="info-legenda">
                ...
                <div class="item">
                    <span class="cor" style="background: #95a5a6;"></span>
                    <span><strong>Cinza:</strong> Valor zero</span>
                </div>
                <div class="item">
            ...
            <div style="margin-top: 20px; background: #f8fafc; border-radius: 12px; padding: 15px 20px; border: 1px solid #eef2f7;">
                <div style="display: flex; flex-wrap: wrap; gap: 20px; font-size: 13px; color: #64748b;">
                    ...
                    <div>
                        <span style="background: #fef9e7; padding: 2px 8px; border-radius: 4px; color: #7a6a2a;">
                            <strong>⚠️</strong> Desconto total limitado ao salário (não fica negativo)
                        </span>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function imprimirMapaSalarios() {
            ...
            janela.document.write('.valor-zero { color: #95a5a6; }');
            ...
            janela.document.write('.aviso { background: #fef9e7; padding: 8px 15px; border-radius: 8px; margin: 10px 0; font-size: 12px; color: #7a6a2a; text-align: center; border: 1px solid #f5d76e; }');
            ...
            janela.document.write('<div class="aviso">⚠️ O desconto total é limitado ao valor do salário para evitar valores negativos</div>');
